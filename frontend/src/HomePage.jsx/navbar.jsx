...
export default function Navbar() {
    ...
    return (
        <>
            <div className=" print:hidden! w-full py-2 bg-[#ff3f39] text-white flex justify-center">
                <p className="m-0 text-center text-nowrap overflow-hidden"><b>Forbes Asia</b> names MGC to its 100 to watch 2025 list </p>
            </div>
            <div className=" print:hidden! shadow-2xl shadow-black/30 bg-white  border-gray-500 sticky top-0 z-50 text-gray-100 w-full flex justify-center">
                <div className="w-306 flex justify-between px-3 py-3">
                    ...
                    <div className="gap-2 text-md font-bold hidden xl:flex">
                        ...
                        }} className="bg-[#d10115] rounded-4xl! px-9 text-lg! hover:border-gray-600! transition-all duration-300 ease-in-out hover:bg-[#ff3f39]! hover:text-black text-white">
            ...
            <>
                <div className={`${toggle ? 'left-0' : 'left-[-100vw]'} ${toggle ? 'md:left-0' : 'md:left-[-33vw]'} w-full md:w-[27vw] transition-all duration-700 ease-in-out bg-white h-screen flex flex-col py-3 gap-0 items-start text-[#041026] fixed left-0 top-24! sm:top-30! border-t border-gray-100 z-12`}>
                    ...
                    <button onClick={() => {navigate('/'),setToggle(!toggle)}} className="border-b border-gray-200 w-full flex justify-between py-3 pl-4 font-medium text-lg">Home</button>
                    <button onClick={() => {navigate('/about-us'),setToggle(!toggle)}} className="border-b border-gray-200 w-full flex justify-between py-3 pl-4 font-medium text-lg">About Us</button>
                    {/* <button className="border-b border-gray-200 w-full flex justify-between py-3 pl-4 font-medium text-lg">Services</button> */}
                    <button onClick={() => {navigate('/contact-us'),setToggle(!toggle)}} className="border-b border-gray-200 w-full flex justify-between py-3 pl-4 font-medium text-lg">Contact Us</button>
                    <button onClick={() => {navigate('/tracking-id'),setToggle(!toggle)}} className="hover:bg-[#d10115] bg-[#ff3f39] mt-4 text-lg! ml-3! py-4 px-5 text-white flex justify-between font-medium">Tracking Id</button>
                </div>
                {
                    toggle ?
