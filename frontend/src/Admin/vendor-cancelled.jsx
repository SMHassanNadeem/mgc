...
export default function OrdersCancelledByVendor() {
    const navigate = useNavigate()
    const queryClient = useQueryClient()

    async function getOrdersVendorCancelled() {
        ...
        const data = await fetch('http://localhost:3000/orders/orders-cancelled-by-vendor', {
            method: 'GET',
            headers: {
    ...
    const { data: apiData, isLoading: ordersLoading, error: ordersError } = useQuery({
        ...
        queryFn: getOrdersVendorCancelled,
        retry: 1,
                        ...
                        <table className="bg-white border w-full rounded-lg shadow-md">
                            <thead className="text-white bg-[#041026]">
                                <tr>
                                    ...
                                    <td className="p-4 text-center text-nowrap font-semibold">Reason of Cancel </td>
                                </tr>
                            </thead>
                            <tbody>
                                {filteredData?.map((a) => (
